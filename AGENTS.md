...
- `anforanden` (nya för Insikt): debattinlägg ur `anforandelista` (data.riksdagen.se), inlästa via `ingestAnforanden` (`typ=anforanden` på inläsningshooken/`korInlasning`). `video_url` är direktlänk till Riksdagens webb-tv med `?pos=<sek>&autoplay=true` när matchningen mot debattsidans `startPosition`-metadata är entydig (per ledamot, lika antal inlägg), annars bara debattens URL. `arenden.debatt_url`/`antal_anforanden` denormaliseras.
